...
if (!empty($_POST)){
  ...
  foreach($questions as $question){
    ...
    //DBにINSERT
    $message1=$db->prepare('INSERT INTO Answers SET answer_id=?, questionnaire_id=?, question_id=?, answer=?, coordinates=?, created_at=NOW()');
    $message1->execute(array($answer_id, $questionnaire_id, $question['question_id'], $answer, $_POST["coordinates"]));

    $message2=$db->prepare('INSERT INTO AnswerDara SET answer_id=?, questionnaire_id=?, scroll=?, coordinates=?, click=?, windowsize=?, background=?, checking=?, type=?, enter_leave=?, total=?, created_at=NOW()');
